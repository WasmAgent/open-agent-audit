...
/**
 * Report metadata fields that control report content and compliance status.
 *
 * These fields influence the framework compliance mapping in the generated report:
 * - `profiles_applied` determines which compliance frameworks are evaluated.
 * - `intended_use` + `deployment_context` populate EU AI Act Annex IV requirements.
 * - `transparency_statement` maps to EU AI Act Art. 13 transparency obligations.
 * - `qms_reference` maps to EU AI Act Art. 17 quality management.
 * - `aep_provenance` contributes to the provenance_integrity EAS component.
 * - `crypto_summary` determines the evidence quality badge and hash chain display.
 *
 * All fields are optional. When omitted, the engine uses sensible defaults
 * and logs a warning for critical fields like `issuer` and `report_url`.
 */
export interface ReportMeta {
  /** Unique report identifier, e.g. "OAA-2026-001". Auto-generated if not provided. */
  report_id?: string;
  /** Issuing organisation / platform name shown in the report. Default: "OpenAgentAudit (self-hosted)" */
  issuer?: string;
  /** Issuer contact email. Default: "noreply@localhost" */
  issuer_email?: string;
...
/**
 * Generate a simple inline SVG QR-code placeholder.
 *
 * This produces a deterministic visual pattern derived from the URL rather than
 * fetching from an external service (fixes #80: no dependency on api.qrserver.com).
 * The pattern is NOT a scannable QR code — it is a visual placeholder that
 * encodes a subset of the URL into a grid. For a fully scannable QR code,
 * consumers should plug in a dedicated QR library (e.g. `qrcode` npm package)
 * and pass the resulting SVG string via a future `qrCodeSvg` meta option.
 */
function generateQrPlaceholderSvg(url: string): string {
  ...
  // Generate a deterministic bit pattern from the URL
  let hash = 0;
  ...
  for (let row = 0; row < modules; row++) {
    for (let col = 0; col < modules; col++) {
      ...
      if (inFinderTL || inFinderTR || inFinderBL) {
        ...
        // Deterministic data pattern from URL hash
        const seed = ((hash >>> 0) ^ (row * 31 + col * 17)) >>> 0;
        dark = seed % 3 !== 0;
      }
...
function resolveMeta(
  ...
  const report_url = meta?.report_url ?? `https://localhost/reports/${report_id}`;

  if (!meta?.issuer) {
    console.warn(
      '[OpenAgentAudit] ReportMeta.issuer not provided — using default placeholder. Set meta.issuer to customise the report issuer.',
    );
  }
  if (!meta?.report_url) {
    console.warn(
      '[OpenAgentAudit] ReportMeta.report_url not provided — using localhost placeholder. Set meta.report_url to point to your hosted report.',
    );
  }
...
function buildComplianceMappings(
  ...
  const denyPolicyEvents = policyDecisionEvents.filter((e) => e.policy?.decision === 'deny');
  ...
  // AAI01 — Memory Poisoning
  {
    ...
    const limitation =
      'Detects only what is present in the trace; out-of-band memory channels are not visible.';
  ...
  // AAI02 — Tool Misuse
  {
    ...
    if (toolCallEvents.length > 0 && policyDecisionEvents.length > 0) {
      ...
      owaspReqs.push({
        ...
        status: 'partial',
        evidence_event_ids: [
          ...new Set([...toolCallEvents.map((e) => e.event_id), ...findingEvIds]),
        ],
        limitation,
      });
    } else {
  ...
  // AAI03 — Privilege Escalation
  {
    ...
    const limitation =
      'Excessive agency may emerge from chained tool calls not visible to a single rule.';
  ...
  // AAI06 — Data Exfiltration
  {
    ...
    const limitation =
      'Detection relies on taint labels being present in the trace. Labels are set by the AEP emitter at run time.';
    ...
    const taintedEvents = toolCallEvents.filter((e) => {
      const tags = e.tool?.risk_tags ?? [];
      return tags.some((t) => OUTPUT_TAINT_INDICATORS.some((ind) => t.toLowerCase().includes(ind)));
    });
  ...
  // AAI07 — Goal Drift — partial when error events or unexpected tool patterns exist
  {
    ...
    const limitation =
      'Full drift detection requires the drift-guard engine across multiple runs. Error events and unexpected tool invocations are a weak proxy for unintended autonomy within a single run.';
    ...
    if (errorEvents.length > 0 || (unexpectedToolEvents.length > 0 && findingEvIds.length > 0)) {
      owaspReqs.push({
        ...
        label,
        status: 'partial',
        evidence_event_ids: [...new Set([...errorEvents.map((e) => e.event_id), ...findingEvIds])],
        limitation,
      });
    } else if (toolCallEvents.length > 0) {
      owaspReqs.push({
  ...
  // AAI08 — Prompt Injection — partial when observation events from tool returns exist
  {
    ...
    const limitation =
      'Signed observation events from tool returns provide forensic traceability but do not prove injection filtering was applied. A dedicated injection-filter verifier result would upgrade this to supported.';
    ...
    const injectionFilterObs = observationEvents.filter(
      (e) =>
        e.observation?.source !== undefined &&
        (e.observation.source.includes('inject') ||
          e.observation.source.includes('filter') ||
          e.observation.source.includes('sanitize') ||
          e.observation.source.includes('guard')),
    );
  ...
  // AAI09 — Insufficient Human Oversight
  {
    ...
    if (highRiskToolCallEvents.length === 0) {
      ...
      owaspReqs.push({
        ...
        status: 'partial',
        evidence_event_ids: [
          ...new Set([...highRiskToolCallEvents.map((e) => e.event_id), ...findingEvIds]),
        ],
        limitation,
      });
    }
    ...
    if (eventsWithEvidence.length === 0) {
      ...
      if (hashCoverage > 0.5) {
        owaspReqs.push({
          ...
          status: 'supported',
          evidence_event_ids: [
            ...new Set([...eventsWithHash.map((e) => e.event_id), ...findingEvIds]),
          ],
          limitation,
        });
      } else {
        owaspReqs.push({
          ...
          status: 'partial',
          evidence_event_ids: [
            ...new Set([...eventsWithEvidence.map((e) => e.event_id), ...findingEvIds]),
          ],
          limitation,
        });
      }
  ...
  // annex-iv-system-description
  {
    ...
    const limitation =
      'A system card supports the requirement but does not by itself satisfy legal sufficiency.';
  ...
  // annex-iv-design-specifications (Annex IV Item 2)
  {
    ...
    const hasDesignContext =
      meta?.intended_use !== undefined &&
      meta.intended_use.trim() !== '' &&
      meta?.deployment_context !== undefined &&
      meta.deployment_context.trim() !== '';
    if (hasDesignContext) {
      euReqs.push({
        id,
        label,
        ...
        evidence_event_ids: toolCallEvents.slice(0, 5).map((e) => e.event_id),
        limitation:
          'ReportMeta fields (intended_use, deployment_context) combined with runtime tool inventory satisfy Annex IV Item 2 at deployment level. Development-phase documentation (training methodology, architecture rationale) remains organisational.',
      });
    } else {
      euReqs.push({
        id,
        ...
        evidence_event_ids: [],
        limitation:
          'Capability manifest not always present; runtime evidence reflects deployed configuration, not development provenance. Populate ReportMeta.intended_use and deployment_context to upgrade to supported.',
      });
    }
  }
  ...
  // annex-iv-risk-management
  {
    ...
    const limitation =
      'Policy decision events show runtime enforcement but Art. 9 requires a documented continuous risk management system (FMEA, risk register) — organizational documentation is required.';
  ...
  // annex-iv-data-governance (Annex IV Item 4, Art. 10)
  {
    ...
    const limitation =
      'Art. 10 data governance requires design-time documentation. Runtime trace shows taint labels and input refs which evidence data handling, not data provenance.';
    const taintedInputEvents = toolCallEvents.filter((e) =>
      (e.tool?.risk_tags ?? []).some(
        (t) =>
          t.toLowerCase().includes('user-supplied') ||
    ...
    if (benchmarkResult !== undefined) {
      ...
      if (verdict === 'supports_claim' && benchmarkResult.findings.length === 0) {
        euReqs.push({
          ...
          limitation:
            'Benchmark statistics support the stated claim with no regressions. Annex IV Item 7 also requires documented test protocols; statistical results alone do not satisfy legal sufficiency.',
        });
      } else {
        euReqs.push({
          id,
      ...
      euReqs.push({
        ...
        evidence_event_ids: [],
        limitation:
          'Requires benchmark engine output. Pass a BenchmarkAuditResult to renderReport to enable this mapping.',
      });
    }
  }
  ...
  // annex-iv-lifecycle-changes (Annex IV Item 6)
  {
    ...
    const limitation =
      'AEP run-provenance fields (repo_commit, runtime_version, policy_bundle_digest, tool_manifest_digest) anchor the record to the exact version in effect at run time. A formal change log across deployments requires organizational documentation.';
    const eventsWithVersion = events.filter(
      (e) =>
        e.evidence?.signer_key_id !== undefined &&
        e.evidence.signer_key_id !== '' &&
    ...
    const allSigned =
      eventsWithVersion.length > 0 &&
      eventsWithVersion.length === eventsWithEvidence.length &&
      eventsWithEvidence.every(
        (e) => e.evidence?.signature !== undefined && e.evidence.signature !== '',
      );
    if (allSigned) {
      euReqs.push({
  ...
  euReqs.push({
    ...
    evidence_event_ids: events.slice(0, 3).map((e) => e.event_id),
    limitation:
      'Trace covers one run; post-market monitoring requires ongoing data collection across deployments.',
  });
  ...
  // annex-iv-human-oversight (Art. 14)
  {
    ...
    const limitation =
      'Approval records do not establish reviewer competence. Art. 14(4) requires humans to understand system capabilities; this cannot be evidenced by trace alone.';
  ...
  // annex-iv-intended-use (Annex IV Item 1(b)) — Art. 13 transparency
  {
    ...
    const limitation =
      'Requires a system card or provider declaration; runtime trace alone cannot substitute. Populate ReportMeta.intended_use to upgrade to partial.';
    ...
    const hasSystemIdentity = events.some(
      (e) => e.agent_id !== '' && e.model_id !== '' && e.model_id !== 'unknown',
    );
    ...
    if (hasIntendedUse && hasSystemIdentity) {
      euReqs.push({
        ...
        limitation:
          'ReportMeta.intended_use and system identity in trace together satisfy Annex IV 1(b) and Art. 13 intent declaration. Legal sufficiency requires a provider-signed declaration.',
      });
  ...
  // annex-iv-logging-capability (Annex IV / Art. 12)
  {
    ...
    const limitation =
      'The presence of a signed, hash-chained trace demonstrates Art. 12(1) logging capability. Art. 12 also requires logging across the full operational lifetime; a single run cannot establish continuity.';
    const allSigned =
  ...
  // annex-iv-qms (Art. 17) — quality management system reference
  {
    ...
    const limitation =
      'QMS documentation is organizational; runtime trace cannot evidence its existence.';
    euReqs.push({ id, label, status: 'not_evaluated', evidence_event_ids: [], limitation });
  }
  ...
  // annex-iv-transparency (Art. 13) — transparency to deployers and users
  {
    ...
    const limitation =
      'Art. 13 requires provider-supplied instructions for use; runtime trace cannot verify disclosure reached end users.';
    euReqs.push({ id, label, status: 'not_evaluated', evidence_event_ids: [], limitation });
  }
  ...
  nistReqs.push({
    ...
    evidence_event_ids: [],
    limitation:
      'Technical artifacts do not establish that an organization has performed the necessary legal review.',
  });
  ...
  // MAP-2.2 — partial if risk_tags present on tool events
  {
    ...
    const taggedToolEvents = toolCallEvents.filter((e) => (e.tool?.risk_tags ?? []).length > 0);
  ...
  // MEASURE-2.7 — supported if findings present (EAS computed); partial otherwise
  {
    ...
    const limitation =
      'Evidence reflects observed runs only; adversarial coverage depends on the test set.';
    const findingEvIds = evidenceFromRules([
      'OAA-R-CAP-001',
      'OAA-R-CAP-002',
      'OAA-R-OVERSIGHT-001',
  ...
  // MEASURE-2.9
  {
    ...
    if (benchmarkResult !== undefined) {
      const { candidate_rate, baseline_rate, absolute_delta, verdict, wilson_ci } =
      ...
      nistReqs.push({
        id,
        label,
        status: verdict !== 'inconclusive' ? 'supported' : 'partial',
  ...
  // Human approvals + deny decisions together suggest an active response posture (partial).
  {
    ...
    const limitation =
      'Runtime trace can show that risk responses were triggered (deny, escalate) but cannot verify that documented recovery procedures exist or were followed.';
  ...
  // MEASURE-2.3 — AI system performance metrics are identified and communicated
  {
    ...
    const limitation =
      'EAS score and per-component breakdown are quantitative performance metrics; they reflect audit-trail quality, not model accuracy or fairness metrics.';
    ...
    const hasPerformanceSignal =
      findings.length > 0 || (toolCallEvents.length > 0 && policyDecisionEvents.length > 0);
  ...
  // MAP-4.1 — risk findings are communicated
  {
    ...
    const limitation =
      'Policy-audit findings in this report evidence risk communication within the audit workflow; they do not confirm that findings were acted on by relevant stakeholders.';
    if (findings.length > 0) {
      nistReqs.push({
        id,
        label,
  ...
  // MEASURE-2.2 — AI system metrics are tracked
  {
    ...
    const limitation: string =
      'The EAS score and per-component breakdown constitute a quantitative evaluation of audit-trail quality per run. Cross-run trend analysis requires ongoing monitoring.';
  ...
  // MANAGE-2.2 — anomaly and error monitoring
  {
    ...
    const limitation =
      'Error and denial events evidence reactive risk responses; they do not prove proactive monitoring cadence.';
  ...
  // MANAGE-4.2 — incident and error logging
  {
    ...
    const limitation =
      'Error events and denied policy decisions are structured incident proxies but do not constitute a formal incident report.';
  ...
  // MAP-5.1 — stakeholder and impact awareness
  {
    ...
    const limitation =
      'Risk characterisation requires human judgment. Taint labels and risk_tags are a partial proxy for impact magnitude.';
  ...
  // MAP-3.2 — context-specific risk identification
  {
    ...
    const limitation =
      'Findings from policy-audit rules reflect known risk patterns; emergent risks outside the rule set are not captured.';
    const findingEvIds = evidenceFromRules([
      'OAA-R-CAP-001',
      'OAA-R-CAP-002',
      'OAA-R-OVERSIGHT-001',
  ...
  // MEASURE-2.5 — output and downstream impact
  {
    ...
    const limitation =
      'State-changing tool calls with post_state_digest evidence show downstream impact; human approval records show oversight configuration.';
  ...
  // MEASURE-2.4 — AI system performance within context
  {
    ...
    const limitation =
      'Tool inventory and policy decisions evidence the AI system operating in its deployment context; full contextual performance evaluation requires defined acceptance criteria.';
    const contextEvIds = [
      ...new Set([
        ...toolCallEvents.slice(0, 3).map((e) => e.event_id),
        ...policyDecisionEvents.slice(0, 3).map((e) => e.event_id),
  ...
  // MEASURE-2.6 — AI system bias, fairness, equity evaluated
  {
    ...
    const limitation =
      'Taint labels (pii, user-supplied) are a proxy for demographic sensitivity; quantitative fairness evaluation requires a dedicated fairness engine and demographic ground-truth data.';
    const biasEvIds = toolCallEvents
      .filter((e) => {
        const tags = e.tool?.risk_tags ?? [];
        return tags.some((t) =>
          ['pii', 'sensitive', 'personal', 'demographic', 'user-supplied'].some((k) =>
  ...
  // MEASURE-2.8 — risks to civil liberties identified and evaluated
  {
    ...
    const label =
      'Risks to civil liberties from AI use are identified, prioritised, and documented';
    const limitation =
      'State-changing tool calls and deny decisions are proxies for civil-liberty-relevant actions; comprehensive civil liberties risk mapping requires human-authored risk register entries.';
    const civilRiskEvIds = [
      ...new Set([
        ...denyPolicyEvents.map((e) => e.event_id),
        ...toolCallEvents
          .filter((e) => e.tool?.risk_tags && e.tool.risk_tags.length > 0)
          .slice(0, 3)
          .map((e) => e.event_id),
      ]),
    ];
    if (denyPolicyEvents.length > 0 || civilRiskEvIds.length > 0) {
      nistReqs.push({
        ...
        label,
        status: 'partial',
        evidence_event_ids: civilRiskEvIds,
        limitation,
      });
    } else {
      nistReqs.push({ id, label, status: 'not_evaluated', evidence_event_ids: [], limitation });
  ...
  // MEASURE-2.10 — privacy risk to individuals identified and prioritised
  {
    ...
    const limitation =
      'PII and user-supplied taint labels evidence data-privacy risk handling at run time; formal privacy risk assessment requires organisational legal review.';
    const privacyEvIds = toolCallEvents
      .filter((e) => {
        const tags = e.tool?.risk_tags ?? [];
        return tags.some((t) =>
          ['pii', 'personal', 'sensitive', 'user-supplied'].some((k) =>
            t.toLowerCase().includes(k),
          ),
        );
      })
  ...
  // MANAGE-1.3 — responses to risks are documented
  {
    ...
    const limitation =
      'Policy-audit findings with recommendations evidence documented risk responses within this report; organisational risk treatment decisions require separate documentation.';
    const findingEvIds = evidenceFromRules([
      'OAA-R-CAP-001',
      'OAA-R-CAP-002',
      'OAA-R-OVERSIGHT-001',
    ...
    if (verifierObs.length > 0 && (denyPolicyEvents.length > 0 || findings.length > 0)) {
      nistReqs.push({
        ...
        evidence_event_ids: [
          ...new Set([
            ...verifierObs.map((e) => e.event_id),
            ...denyPolicyEvents.slice(0, 2).map((e) => e.event_id),
          ]),
        ],
        limitation,
      });
  ...
  // MAP-1.5 — organisational context documented for AI deployment
  {
    ...
    const limitation =
      'ReportMeta fields (intended_use, deployment_context, source_files) document the deployment context; formal organisational AI use policy requires governance documentation.';
    ...
    if (hasDeploymentContext) {
      ...
      nistReqs.push({
        ...
        evidence_event_ids: events.slice(0, 3).map((e) => e.event_id),
        limitation:
          'Runtime trace documents AI system operation. Populate ReportMeta.intended_use and deployment_context for stronger evidence.',
      });
    } else {
      nistReqs.push({ id, label, status: 'not_evaluated', evidence_event_ids: [], limitation });
    }
  ...
  // MAP-2.3 — AI system scientific and technical approaches documented
  {
    ...
    const limitation =
      'AEP run-provenance fields (repo_commit, runtime_version, policy_bundle_digest) document the technical version in use; scientific methodology documentation requires design-time artifacts.';
  ...
  nistReqs.push({
    ...
    evidence_event_ids: [],
    limitation:
      'Drift detection is statistical; semantic drift is harder. Requires ongoing monitoring data.',
  });
  ...
  // MEASURE-2.11 — bias and fairness monitoring
  {
    ...
    const limitation =
      'Taint labels (pii, user-supplied) are a proxy for sensitive-data handling; demographic bias measurement requires dedicated fairness engine output.';
    const sensitiveDataEvents = toolCallEvents.filter((e) => {
      const tags = e.tool?.risk_tags ?? [];
      return tags.some((t) =>
        ['pii', 'sensitive', 'personal', 'user-supplied'].some((k) => t.toLowerCase().includes(k)),
      );
    });
  ...
  const a73EvIds = observationEvents
    .filter((e) => e.observation?.source?.startsWith('verifier:'))
    .map((e) => e.event_id);
  ...
  const a75EvIds = [
    ...new Set([
      ...
      ...denyPolicyEvents.map((e) => e.event_id),
      ...events
        .filter((e) => e.evidence?.hash !== undefined)
        .slice(0, 5)
        .map((e) => e.event_id),
    ]),
  ];
  ...
  const a85EvIds = toolCallEvents
    .filter((e) => {
      const tags = e.tool?.risk_tags ?? [];
      return tags.some((t) =>
        ['user-supplied', 'external', 'input'].some((k) => t.toLowerCase().includes(k)),
      );
    })
    .map((e) => e.event_id);

  // A.9.1 — third-party AI use
  const a91EvIds = events
    .filter((e) => e.evidence?.signer_key_id && e.evidence.signer_key_id !== 'none')
    .map((e) => e.event_id);
  ...
  const isoReqs: ReqEntry[] = [
    ...
    },
    {
      ...
      ...(a62EvIds.length > 0
        ? {
            ...
            evidence_event_ids: a62EvIds,
            limitation:
              'Taint labels on tool calls indicate sensitive data handling; demographic impact assessment requires dedicated fairness analysis.',
          }
        : {
            status: 'not_evaluated' as const,
            evidence_event_ids: [],
            limitation: 'No sensitive data taint labels detected in this trace.',
          }),
    },
    {
      id: 'A.7.3',
      ...
      ...(a73EvIds.length > 0
        ? {
            status: 'partial' as const,
            evidence_event_ids: a73EvIds,
            limitation:
              'Verifier result observations evidence V&V execution but not test coverage or acceptance criteria adequacy.',
          }
        : {
            status: 'not_evaluated' as const,
            evidence_event_ids: [],
            limitation: 'No verifier result observations present in this trace.',
          }),
    },
    {
      id: 'A.7.4',
    ...
    },
    {
      ...
      ...(() => {
        const hashCoverage100 =
          events.length > 0 &&
          events.every((e) => e.evidence?.hash !== undefined && e.evidence.hash !== '');
        const hasSignatures = events.some(
          (e) => e.evidence?.signature !== undefined && e.evidence.signature !== '',
        );
    ...
    },
    {
      ...
      ...(benchmarkResult !== undefined
        ? {
            status: (benchmarkResult.statistics.verdict !== 'inconclusive'
              ? 'supported'
    ...
    },
    {
      ...
      ...(a83EvIds.length > 0
        ? {
            status: 'partial' as const,
            evidence_event_ids: a83EvIds.slice(0, 5),
            limitation:
              'Risk-tagged tool calls evidence data handling checkpoints; schema validation requires dedicated data-quality verifier observations.',
          }
        : {
            status: 'not_evaluated' as const,
            evidence_event_ids: [],
            limitation: 'No risk-tagged tool events detected.',
          }),
    },
    {
      id: 'A.8.5',
      ...
      ...(a85EvIds.length > 0
        ? {
            status: 'partial' as const,
            evidence_event_ids: a85EvIds,
            limitation:
              'User-supplied and external taint labels evidence runtime input monitoring; distribution drift detection requires baseline comparison data.',
          }
        : {
            status: 'not_evaluated' as const,
            evidence_event_ids: [],
            limitation: 'No external/user-supplied input events detected in this trace.',
          }),
    },
    {
      id: 'A.9.1',
    ...
    },
    {
      ...
      ...(meta?.intended_use !== undefined && meta.intended_use.trim() !== ''
        ? {
            ...
            evidence_event_ids: toolCallEvents.slice(0, 3).map((e) => e.event_id),
            limitation:
              'ReportMeta.intended_use provides design-intent documentation. Formal design objectives (architecture rationale, constraint decisions) remain organisational artifacts.',
          }
        : {
            status: 'not_evaluated' as const,
            evidence_event_ids: [],
            limitation:
              'Populate ReportMeta.intended_use to partially satisfy A.7.2 design objectives evidence.',
          }),
    },
    {
      id: 'A.8.4',
      ...
      ...(() => {
        ...
        if (inputRefsInTrace.length > 0) {
          return {
            status: 'partial' as const,
            evidence_event_ids: inputRefsInTrace.map((e) => e.event_id),
            limitation:
              'Input observations evidence runtime data handling; formal data preprocessing provenance requires design-time pipeline documentation.',
          };
        }
        return {
          status: 'not_evaluated' as const,
          evidence_event_ids: [],
          limitation:
            'No data preparation signals detected. Add input-tagged observation events to evidence data preprocessing.',
        };
      })(),
    },
    {
      ...
      ...(a91EvIds.length > 0 && (errorEvents.length > 0 || denyPolicyEvents.length > 0)
        ? {
            ...
            evidence_event_ids: [
              ...new Set([
                ...a91EvIds.slice(0, 3),
                ...errorEvents.slice(0, 2).map((e) => e.event_id),
                ...denyPolicyEvents.slice(0, 2).map((e) => e.event_id),
              ]),
            ],
            limitation:
              'Signer key IDs identify the third-party component; error/deny events evidence active monitoring. Contractual SLA monitoring requires organisational documentation.',
          }
        : {
            status: 'not_evaluated' as const,
            evidence_event_ids: [],
            limitation:
              'Requires both third-party signer identity and monitoring signal events (error or deny).',
          }),
    },
    {
      id: 'A.5.2',
      ...
      ...(meta?.qms_reference !== undefined && meta.qms_reference.trim() !== ''
        ? {
            ...
            evidence_event_ids: [],
            limitation:
              'ReportMeta.qms_reference documents the AI policy framework. Policy communication to stakeholders requires organisational verification.',
          }
        : {
            status: 'not_evaluated' as const,
            evidence_event_ids: [],
            limitation:
              'Populate ReportMeta.qms_reference to partially satisfy A.5.2 AI policy documentation.',
          }),
    },
    {
      id: 'A.10.1',
      ...
      ...(meta?.source_files !== undefined && (meta.source_files as string[]).length > 0
        ? {
            ...
            evidence_event_ids: events.slice(0, 3).map((e) => e.event_id),
            limitation:
              'Source file name and audit trace document the operational run. Comprehensive operational documentation (deployment runbooks, incident procedures) requires organisational artifacts.',
          }
        : {
            status: 'not_evaluated' as const,
            evidence_event_ids: [],
            limitation:
              'Supply a source file name via ReportMeta.source_files or X-Source-File header to partially satisfy A.10.1.',
          }),
    },
  ];
...
function buildCsv(events: CanonicalEvent[], findings: Finding[]): string {
  const rows: string[] = [];
  ...
  for (const profile of mappings) {
    ...
    if (profile.profile_id === 'owasp-agentic-top10-2026') {
      ...
      for (const req of profile.requirements) {
        const evIds = req.evidence_event_ids.length > 0 ? req.evidence_event_ids.join(', ') : '—';
        lines.push(`| ${req.id} | ${req.label} | ${statusSymbol(req.status)} | ${evIds} |`);
      }
    } else {
      ...
      for (const req of profile.requirements) {
        ...
        lines.push(`| ${req.id} | ${req.label} | ${statusSymbol(req.status)} | ${limitation} |`);
      }
    }
    lines.push('');
  }
...
function buildEventDetails(ev: CanonicalEvent): string {
  if (ev.tool?.name) {
    ...
    const tags =
      ev.tool.risk_tags && ev.tool.risk_tags.length > 0 ? ` [${ev.tool.risk_tags.join(', ')}]` : '';
...
function buildMarkdown(
  ...
  const retentionAnchor =
    resolved.trace_end !== ''
      ? resolved.trace_end > generatedAt
        ? resolved.trace_end
        : generatedAt
      : generatedAt;
  ...
  if (resolved.deployment_context)
    lines.push(`| Deployment context | ${resolved.deployment_context} |`);
  if (resolved.transparency_statement)
    lines.push(`| Transparency statement | ${resolved.transparency_statement} |`);
  ...
  if (prov !== undefined && Object.keys(prov).length > 0) {
    ...
    if (prov.policy_bundle_digest)
      lines.push(`| Policy bundle digest | \`${prov.policy_bundle_digest.slice(0, 16)}…\` |`);
    if (prov.tool_manifest_digest)
      lines.push(`| Tool manifest digest | \`${prov.tool_manifest_digest.slice(0, 16)}…\` |`);
    if (prov.mcp_server_card_digest)
      lines.push(`| MCP server card digest | \`${prov.mcp_server_card_digest.slice(0, 16)}…\` |`);
  ...
  lines.push(`| Agent Risk Score (ARS) | ${agent_risk_score.score}/100 |`);
  ...
  const verifierObs = events.filter(
    (e) => e.type === 'observation' && e.observation?.source?.startsWith('verifier:'),
  ).length;
  lines.push(
    `| Trace Completeness | ${tcScore}/100 | Penalties for missing evidence_id or timestamp fields, and unpaired tool calls |`,
  );
  lines.push(
    `| Provenance Integrity | ${piScore}/100 | Hash chain integrity and Ed25519 signature coverage across all events |`,
  );
  lines.push(
    `| Objective Verification | ${ovScore}/100 | ${verifierObs} verifier result(s) against ${toolCalls} tool call(s) — deterministic verifier coverage |`,
  );
  lines.push(
    `| Policy Coverage | ${pcScore}/100 | ${policyDecisions} policy decision(s) against ${toolCalls} tool call(s) — ${toolCalls > 0 ? Math.round((policyDecisions / toolCalls) * 100) : 'N/A'}% coverage |`,
  );
  lines.push(
    `| Human Oversight Evidence | ${hoScore}/100 | Human approval records for actions tagged high_risk or human_required |`,
  );
  lines.push(
    `| Contamination Risk | ${crScore}/100 | Training/test data overlap risk (100 = no contamination detected) |`,
  );
  if (!score.contamination_evaluated) {
    lines.push('');
    lines.push(
      '> **Note:** The Contamination Risk component reflects a neutral score (no training event set was provided). To evaluate real contamination, pass a `ContaminationResult` to `computeRiskScore()`.',
    );
    lines.push('');
  }
  lines.push(
  ...
  if (cryptoSummary !== undefined) {
    const mismatchNote =
      cryptoSummary.hashes_content_mismatch > 0
  ...
  if (findings.length === 0) {
    ...
    for (const f of sorted) {
      ...
      if (f.standard_mappings !== undefined && f.standard_mappings.length > 0) {
        const mappings = f.standard_mappings.map((m) => `${m.profile}:${m.control_id}`).join(', ');
  ...
  lines.push('');
  lines.push(
    '- This report covers only events present in the submitted trace. Behavior outside this trace is not evaluated.',
  );
  lines.push('- Regulatory framework mappings are interpretive and non-binding. See Disclaimer.');
  lines.push(
    '- Evidence integrity is verified only for events that include hash/signature fields.',
  );
  lines.push(
    '- Tool capability declarations are taken at face value; manifest honesty is assumed.',
  );
  lines.push('');
  ...
  const chainEvents = events.filter(
    (e) => e.evidence?.hash !== undefined || e.evidence?.prev_hash !== undefined,
  );
  ...
  lines.push('');
  lines.push(
    `> This appendix lists all ${events.length} events in the submitted trace for forensic reference.`,
  );
  lines.push(
    `> Generated by ${resolved.issuer} — ${resolved.issuer_email} — OpenAgentAudit v${resolved.engine_version}`,
  );
  lines.push('');
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    lines.push(
      `| ${i + 1} | ${decodeEventId(ev.event_id)} | ${ev.type} | ${ev.actor} | ${ev.timestamp} | ${details} | ${evidenceId} |`,
    );
  }
  lines.push('');
...
function buildComplianceMappingHtml(mappings: ComplianceMapping[]): string[] {
  ...
  for (const profile of mappings) {
    ...
    if (profile.profile_id === 'owasp-agentic-top10-2026') {
      parts.push('<table>');
      parts.push(
        '<thead><tr><th>Control</th><th>Label</th><th>Status</th><th>Evidence Events</th></tr></thead>',
      );
      parts.push('<tbody>');
      for (const req of profile.requirements) {
        const evIds =
      ...
      parts.push('<table>');
      parts.push(
        '<thead><tr><th>Requirement</th><th>Description</th><th>Status</th><th>Limitation</th></tr></thead>',
      );
      parts.push('<tbody>');
      for (const req of profile.requirements) {
        const limitation =
...
function buildHtml(
  ...
  const css = `
    ...
    @media print {
      ...
      body::before {
        content: "OPENAGENTAUDIT  •  OFFICIAL REPORT  •  EU AI ACT COMPLIANT  •  ";
        position: fixed;
        top: 48%;
        left: -20%;
        width: 150%;
  ...
  parts.push(
    `<title>Audit Report ${escapeHtml(runId.slice(0, 8))} — ${escapeHtml(resolved.issuer)} — OpenAgentAudit</title>`,
  );
  parts.push(
    `<meta name="description" content="AI agent audit report for run ${escapeHtml(runId.slice(0, 8))}. EAS score: ${evidence_admission_score.score}/100 (Grade ${escapeHtml(grade)}). ${escapeHtml(findingsSummary)}. Issued by ${escapeHtml(resolved.issuer)}.">`,
  );
  ...
  parts.push(
    `<tr><td>Issuer contact</td><td><a href="mailto:${escapeHtml(resolved.issuer_email)}">${escapeHtml(resolved.issuer_email)}</a></td></tr>`,
  );
  parts.push(
    `<tr><td>Report URL</td><td><a href="${escapeHtml(resolved.report_url)}">${escapeHtml(resolved.report_url)}</a></td></tr>`,
  );
  ...
  if (resolved.intended_use)
    parts.push(`<tr><td>Intended use</td><td>${escapeHtml(resolved.intended_use)}</td></tr>`);
  if (resolved.deployment_context)
    parts.push(
      `<tr><td>Deployment context</td><td>${escapeHtml(resolved.deployment_context)}</td></tr>`,
    );
  if (resolved.transparency_statement)
    parts.push(
      `<tr><td>Transparency statement (Art. 13)</td><td>${escapeHtml(resolved.transparency_statement)}</td></tr>`,
    );
  if (resolved.qms_reference)
    parts.push(
      `<tr><td>QMS reference (Art. 17)</td><td>${escapeHtml(resolved.qms_reference)}</td></tr>`,
    );
  parts.push('</tbody>');
  parts.push('</table>');
  ...
  if (prov !== undefined && Object.keys(prov).length > 0) {
    ...
    if (prov.repo_commit)
      parts.push(
        `<tr><td>Repo commit</td><td><code>${escapeHtml(prov.repo_commit)}</code></td></tr>`,
      );
    if (prov.runtime_version)
      parts.push(`<tr><td>Runtime version</td><td>${escapeHtml(prov.runtime_version)}</td></tr>`);
    if (prov.model_provider)
      parts.push(`<tr><td>Model provider</td><td>${escapeHtml(prov.model_provider)}</td></tr>`);
    if (prov.policy_bundle_digest)
      parts.push(
        `<tr><td>Policy bundle digest</td><td><code>${escapeHtml(prov.policy_bundle_digest.slice(0, 16))}…</code></td></tr>`,
      );
    if (prov.tool_manifest_digest)
      parts.push(
        `<tr><td>Tool manifest digest</td><td><code>${escapeHtml(prov.tool_manifest_digest.slice(0, 16))}…</code></td></tr>`,
      );
    if (prov.mcp_server_card_digest)
      parts.push(
        `<tr><td>MCP server card digest</td><td><code>${escapeHtml(prov.mcp_server_card_digest.slice(0, 16))}…</code></td></tr>`,
      );
    if (prov.parent_trace_id)
      parts.push(
        `<tr><td>Parent trace ID</td><td><code>${escapeHtml(prov.parent_trace_id)}</code></td></tr>`,
      );
    if (prov.delegation_chain && prov.delegation_chain.length > 0) {
      parts.push(
        `<tr><td>Delegation chain</td><td>${escapeHtml(prov.delegation_chain.join(' → '))}</td></tr>`,
      );
    }
    parts.push('</tbody>');
    parts.push('</table>');
  ...
  parts.push(`<tr><td>Agent Risk Score (ARS)</td><td>${agent_risk_score.score}/100</td></tr>`);
  ...
  parts.push(
    `<tr><td>Trace Completeness</td><td>${tcScoreH}/100</td><td>Penalties for missing evidence_id or timestamp fields, and unpaired tool calls</td></tr>`,
  );
  parts.push(
    `<tr><td>Provenance Integrity</td><td>${piScoreH}/100</td><td>Hash chain integrity and Ed25519 signature coverage across all events</td></tr>`,
  );
  parts.push(
    `<tr><td>Objective Verification</td><td>${ovScoreH}/100</td><td>${verifierObsH} verifier result(s) against ${toolCallsH} tool call(s) — deterministic verifier coverage</td></tr>`,
  );
  parts.push(
    `<tr><td>Policy Coverage</td><td>${pcScoreH}/100</td><td>${policyDecisionsH} policy decision(s) against ${toolCallsH} tool call(s) — ${toolCallsH > 0 ? Math.round((policyDecisionsH / toolCallsH) * 100) : 'N/A'}% coverage</td></tr>`,
  );
  parts.push(
    `<tr><td>Human Oversight Evidence</td><td>${hoScoreH}/100</td><td>Human approval records for actions tagged high_risk or human_required</td></tr>`,
  );
  parts.push(
    `<tr><td>Contamination Risk</td><td>${crScoreH}/100</td><td>Training/test data overlap risk (100 = no contamination detected)</td></tr>`,
  );
  if (!score.contamination_evaluated) {
    parts.push('</tbody></table>');
    parts.push(
      '<p class="oaa-note"><strong>Note:</strong> The Contamination Risk component reflects a neutral score (no training event set was provided). To evaluate real contamination, pass a <code>ContaminationResult</code> to <code>computeRiskScore()</code>.</p>',
    );
    parts.push(
      '<table><thead><tr><th>Component</th><th>Score</th><th>What this means</th></tr></thead><tbody>',
    );
  }
  parts.push(
  ...
  if (inv !== undefined && Array.isArray(inv.tools)) {
    ...
    parts.push('<table>');
    parts.push(
      '<thead><tr><th>Tool</th><th>Calls</th><th>Denied</th><th>Approved</th><th>Risk Tags</th></tr></thead>',
    );
    parts.push('<tbody>');
    for (const t of inv.tools) {
      const tags = t.risk_tags.length > 0 ? escapeHtml(t.risk_tags.join(', ')) : '<em>none</em>';
      parts.push(
  ...
  parts.push('<ul>');
  parts.push(
    '<li>This report covers only events present in the submitted trace. Behavior outside this trace is not evaluated.</li>',
  );
  parts.push(
    '<li>Regulatory framework mappings are interpretive and non-binding. See Disclaimer.</li>',
  );
  parts.push(
    '<li>Evidence integrity is verified only for events that include hash/signature fields.</li>',
  );
  parts.push(
    '<li>Tool capability declarations are taken at face value; manifest honesty is assumed.</li>',
  );
  parts.push('</ul>');
  ...
  // Official Seal + QR code (inline SVG — no external fetch)
  const qrSvg = generateQrPlaceholderSvg(resolved.report_url);
  const retentionUntilHtml = addSixMonths(generatedAt);
  parts.push(
    '<div class="no-print" style="margin:32px 0;padding:20px;border:1px solid #e5e7eb;border-radius:8px;background:#f9fafb;display:flex;align-items:center;gap:24px">',
  );
  parts.push(qrSvg);
  parts.push('<div>');
  parts.push(
    `<div style="font-size:13px;color:#6b7280;margin-bottom:4px">Scan to access this report online</div>`,
  );
  parts.push(
    `<div style="font-size:12px;font-family:monospace;color:#374151;margin-bottom:8px">${escapeHtml(resolved.report_url)}</div>`,
  );
  parts.push(
    `<div style="font-size:11px;color:#9ca3af">Retained until ${retentionUntilHtml} · EU AI Act Art. 26(6)</div>`,
  );
  parts.push(
    `<div style="font-size:11px;color:#9ca3af">Contact: <a href="mailto:${escapeHtml(resolved.issuer_email)}">${escapeHtml(resolved.issuer_email)}</a></div>`,
  );
  parts.push('</div>');
  parts.push('</div>');
  ...
  const sealSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 160 160">
    ...
    <text x="80" y="52" text-anchor="middle" font-family="Georgia,serif" font-size="9" fill="#4f46e5" letter-spacing="2" font-weight="bold">OPEN AGENT</text>
    <text x="80" y="64" text-anchor="middle" font-family="Georgia,serif" font-size="7" fill="#6b7280" letter-spacing="1">AUDIT</text>
  ...
  if (chainEvs.length > 0) {
    ...
    parts.push('<table>');
    parts.push(
      '<thead><tr><th>Event ID</th><th>Type</th><th>Has Signature</th><th>Chain Status</th></tr></thead>',
    );
    parts.push('<tbody>');
    let prevHashHtml = '';
    ...
    parts.push(
      `<p><em>${chainEvs.length} of ${events.length} events signed. ${unsignedCnt} unsigned.</em></p>`,
    );
  }
  ...
  if (narrativeConclusion !== undefined && narrativeConclusion.trim().length > 0) {
    parts.push(
      `<blockquote>${escapeHtml(narrativeConclusion).replace(/\n/g, '<br>')}</blockquote>`,
    );
  }
  ...
  parts.push(
    `<p><em>All ${events.length} events in the submitted trace. Generated by ${escapeHtml(resolved.issuer)} — ${escapeHtml(resolved.issuer_email)}</em></p>`,
  );
  parts.push('<div style="overflow-x:auto">');
  parts.push('<table style="font-size:11px">');
  parts.push(
    '<thead><tr><th>#</th><th>Event ID</th><th>Type</th><th>Actor</th><th>Timestamp</th><th>Details</th><th>Evidence ID</th></tr></thead>',
  );
  parts.push('<tbody>');
  for (let i = 0; i < events.length; i++) {
    const ev = events[i];
