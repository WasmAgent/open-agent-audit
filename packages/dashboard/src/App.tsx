...
function SummaryCard({
  ...
  return (
    <div
      className={`bg-white rounded-2xl border border-slate-200 border-l-4 ${accent} p-5 shadow-sm hover:shadow-md transition-shadow`}
    >
      ...
      <section className="pt-6 pb-2 flex flex-col items-center text-center gap-5">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm max-w-md mx-auto leading-relaxed">
            Upload a JSONL audit trace to inspect agent events, score evidence quality, and generate
            evidence-linked technical reports that may support selected documentation and oversight
            workflows under relevant AI governance frameworks. Not legal advice.
          </p>
        </div>
        <div className="flex flex-wrap justify-center gap-2">
          {[
...
function AuditPage() {
  ...
  const {
    ...
    sort,
    toggleSort,
  } = useSortable<RawEvent, 'timestamp' | 'type' | 'tool'>(events, sortGetters, 'timestamp', 'asc');
      ...
      <section>
        ...
        {isAepRecord ? (
            ...
            {reportSummary ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="bg-white rounded-xl border border-slate-200 p-3 text-center shadow-sm">
                    ...
                    <div className="text-2xl font-bold text-indigo-600">
                      {reportSummary.eas_score ?? '—'}
                    </div>
                    <div className="text-xs text-slate-500 mt-0.5">
                      Grade {reportSummary.eas_grade ?? '—'}
                    </div>
                  </div>
                  <div className="bg-white rounded-xl border border-slate-200 p-3 text-center shadow-sm">
                    <div className="text-[10px] uppercase tracking-widest text-slate-400 mb-1">
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 p-3 text-center shadow-sm">
                    ...
                    <div
                      className={`text-2xl font-bold ${(reportSummary.finding_count ?? 0) > 0 ? 'text-amber-500' : 'text-emerald-500'}`}
                    >
              ...
              <div>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="font-semibold text-slate-800 text-sm">
                    AEP record loaded: <span className="font-mono text-indigo-600">{fileName}</span>
                  </span>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <div className="bg-white/70 rounded-xl border border-indigo-100 p-3 shadow-sm">
                    ...
                    <div className="text-xs font-mono text-indigo-600">
                      {aepMeta?.schema_version ?? '—'}
                    </div>
                  </div>
                </div>
                <p className="text-xs text-slate-400 mt-3 text-center">
                  Click <strong className="text-slate-600">Generate Report</strong> to run the full
                  audit pipeline.
                </p>
              </div>
            )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <SummaryCard
              label="Run ID"
              value={firstEvent?.run_id ?? '—'}
              accent="border-l-violet-400"
            />
            <SummaryCard
              label="Agent ID"
              value={firstEvent?.agent_id ?? '—'}
              accent="border-l-emerald-400"
            />
            <SummaryCard
              label="Model ID"
              value={firstEvent?.model_id ?? '—'}
              ...
              <table className="min-w-full divide-y divide-slate-100 text-sm">
                <thead className="bg-slate-50 sticky top-0">
                  <tr>
                    ...
                    >
                      Timestamp{' '}
                      {sort.key === 'timestamp' ? (sort.direction === 'asc' ? '↑' : '↓') : ''}
                    </th>
                    <th
                      className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-widest cursor-pointer select-none hover:text-indigo-500 transition-colors"
...
function AppShell() {
  ...
  const crumbs: Crumb[] = (() => {
    ...
    if (location === '/runs') {
      return [{ label: 'Home', href: '/' }, { label: 'Runs' }];
    }
    if (location === '/approvals') {
      return [{ label: 'Home', href: '/' }, { label: 'Approvals' }];
    }
    if (location === '/audit') {
      return [
        { label: 'Home', href: '/' },
    ...
    if (location.startsWith('/runs/')) {
      return [{ label: 'Home', href: '/' }, { label: 'Runs', href: '/runs' }, { label: 'Report' }];
    }
    return [{ label: 'Home' }];
  })();
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <footer className="border-t border-slate-100 bg-white mt-auto">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-400">
          <span>
            <span className="font-medium text-slate-500">{config.site_name}</span> — Technical
